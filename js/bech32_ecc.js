...
function bech32check (bechString, validHrp) {
  ...
  if (pos == -1) {
    return {error:"Missing separator '1'", pos:[]};
  }
  ...
  return {error:null, hrp:hrp, data:data.slice(0, -6), pos:[]};
}
...
function check (addr, validHrp) {
  if (addr.length < 14) {
    return {error:"Too short", pos:[]};
  }
  if (addr.length > 74) {
    return {error:"Too long", pos:[]};
  }
  if ((addr.length % 8) == 0 || (addr.length % 8) == 3 || (addr.length % 8) == 5) {
    return {error:"Invalid length", pos:[]};
  }
  var dec = bech32check(addr, validHrp);
  if (dec.error !== null) {
  ...
  if (res.length < 2 || res.length > 40) {
    return {error:"Invalid witness program length", pos:[]};
  }
  if (dec.data[0] > 16) {
    return {error:"Invalid witness version", pos:[dec.hrp.length + 1]};
  }
  if (dec.data[0] === 0 && res.length !== 20 && res.length !== 32) {
    return {error:"Invalid witness program length for v0", pos:[]};
  }
  return {error:null, version:dec.data[0], program:res, pos:[]};
}
